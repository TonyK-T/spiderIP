...
http_url = ['http://news.163.com/latest/', 'http://fanyi.youdao.com/', 'http://news.qq.com/', 'http://vdisk.weibo.com/',
            'http://www.kugou.com/']
https_url = ['https://www.baidu.com/', 'https://fanyi.baidu.com/', 'https://news.baidu.com/',
             'https://www.toutiao.com/', 'https://www.sogou.com/', 'https://www.xunlei.com/', 'https://www.csdn.net/']
...
class IPCheck:
    ...
    async def aiohttp_check2(self, proxy, url, semaphore):
        ...
        async with semaphore:
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get(url, proxy=proxies,
                                           timeout=2) as resp:  # aiohttp_proxy =  http:// 218.60.8.98:3129
    ...
    def _run_aiohttp(self, aiohttp_tasks):
        '''
         loop 执行 aiohttp
        '''
    ...
    def _run_grequests(self, grequests_tasks):
        '''
        执行 grequests
        '''
        if grequests_tasks:
    ...
    def run_ip_check(self, queue, new_queue):
        '''
        ...
        # 报错待解决： child watchers are only available on the default loop

        self._run_aiohttp(aiohttp_tasks)
        self._run_grequests(grequests_tasks)

        # thread1 = Thread(target=self._run_aiohttp, args=(aiohttp_tasks,))
        # thread2 = Thread(target=self._run_grequests, args=(grequests_tasks,))
        #
